...
class InterfaceJob {
  ...
  void _logSummary(List<String> generatedFiles) {
    logger.info('');
    logger.info('Interface generation summary:');
    ...
    logger.info('');
    logger.success(
      MainConstants.generatedFileSuccess.formatWith({'name': 'interface'}),
    );
  }

  /// Generate files using Mason brick
  Future<List<String>> _generate(InterfaceData data) async {
  ...
  Future<Directory> _prepareTargetDirectory(InterfaceData data) async {
  ...
  List<String> _buildGeneratedFilesList(InterfaceData data) {
    ...
    return [
      '$basePath/${MainConstants.interfaceSuffix}'.formatWith({
        'name': nameSnakeCase,
      }),
    ];
  }
