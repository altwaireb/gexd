...
class LocaleGeneratorService implements LocaleGeneratorServiceInterface {
  ...
  Future<File> generateTranslationsFile({
    ...
    try {
      ...
      // Generate extensions files in the same directory
      await _generateExtensionsFiles(outputFile.parent);

      progress.complete(
        '✓ Generated translations file with ${sortedKeys.length} keys + extensions',
      );
      return outputFile;
    } catch (e) {
  ...
  Map<String, String> flattenJson({
    ...
    void flatten(Map<String, dynamic> obj, [String prefix = '']) {
      obj.forEach((key, value) {
        ...
        if (value is Map<String, dynamic> && value.containsKey('__count')) {
          // Handle pluralization: store as JSON string
          flattened[newKey] = jsonEncode(value['__count']);
  ...
  /// Generate extension files in the translations directory
  Future<void> _generateExtensionsFiles(Directory translationsDir) async {
    // Create extensions subdirectory
    final extensionsDir = Directory(
      path.join(translationsDir.path, 'extensions'),
    );
    await extensionsDir.create(recursive: true);

    // Generate trVars extension
    final trVarsFile = File(
      path.join(extensionsDir.path, 'tr_vars_extension.dart'),
    );
    await trVarsFile.writeAsString(_generateTrVarsExtension());
    logger.detail('✓ Generated tr_vars_extension.dart');

    // Generate trCount extension
    final trCountFile = File(
      path.join(extensionsDir.path, 'tr_count_extension.dart'),
    );
    await trCountFile.writeAsString(_generateTrCountExtension());
    logger.detail('✓ Generated tr_count_extension.dart');
  }

  /// Generate trVars extension content
  String _generateTrVarsExtension() {
    return '''// GENERATED CODE - DO NOT MODIFY BY HAND
// Generated by gexd locale generator
// ${DateTime.now().toIso8601String()}

import 'package:get/get.dart';

/// Extension for named variable replacement in translation strings.
/// 
/// Example usage:
/// ```dart
/// Text('welcome'.trVars({'name': 'Ali'})); // "Welcome Ali"
/// ```
extension TrVarsExtension on String {
  /// Replace named variables in translation string
  /// 
  /// [vars] - Map of variable names to values
  /// Returns the translated string with variables replaced
  String trVars([Map<String, String>? vars]) {
    final locale = Get.locale?.toString() ?? 'en_US';
    final translations = Get.translations[locale] ?? {};
    var text = translations[this]?.toString() ?? this;
...
''';
  ...
  /// Generate trCount extension content
  String _generateTrCountExtension() {
    return '''// GENERATED CODE - DO NOT MODIFY BY HAND
// Generated by gexd locale generator
// ${DateTime.now().toIso8601String()}
...
/// Extension for universal pluralization / count-based translations.
/// Supports JSON format generated from `__count` with all standard plural keys.
/// 
/// Supported keys (use any combination based on your language needs):
/// - zero: exactly 0 items
/// - one: exactly 1 item
/// - two: exactly 2 items  
/// - few: small quantities (3-10 by default, customize as needed)
/// - many: larger quantities (11+ by default, customize as needed)
/// - other: fallback for any other cases
/// 
/// Example usage:
/// ```dart
/// // English
...
/// 
/// // Arabic (with custom few/many ranges)
/// Text('عنصر'.trCount({'count': '3'})); // "3 عناصر" (few)
/// Text('عنصر'.trCount({'count': '15'})); // "15 عنصراً" (many)
/// ```
extension TrCountExtension on String {
  /// Handle pluralization based on count with variable replacement
  /// 
  /// [params] - Map containing count and other variables
  /// Returns the appropriate plural form with variables replaced
  String trCount([Map<String, String>? params]) {
    final locale = Get.locale?.toString() ?? 'en_US';
    final translations = Get.translations[locale] ?? {};
    
    final raw = translations[this];
    if (raw == null) return this;

    Map<String, dynamic>? pluralForms;

    // Try to decode JSON if it's a string (from __count)
    try {
      pluralForms = jsonDecode(raw);
    } catch (_) {
      // Not JSON, treat as regular translation
      return _replaceVariables(raw, params);
    }

    if (pluralForms == null) return this;

    // Get count from parameters
    final countStr = params?['count'] ?? '0';
    final count = int.tryParse(countStr) ?? 0;

    // Determine which plural key to use based on locale and count
    final selectedKey = _getPluralKey(locale, count);
    
    var value = pluralForms[selectedKey]?.toString() ?? 
                pluralForms['other']?.toString() ?? 
                this;

    // Replace all variables including count
    return _replaceVariables(value, params);
  }

  /// Replace variables in text
  String _replaceVariables(String text, Map<String, String>? params) {
  ...
  /// Determine plural key based on count (universal support for all languages)
  /// 
  /// This method provides a universal pluralization system that works with all languages.
  /// Developers can define any combination of these keys in their JSON files:
  /// - zero: exactly 0 items
  /// - one: exactly 1 item  
  /// - two: exactly 2 items
  /// - few: small quantity (typically 3-10, varies by language)
  /// - many: large quantity (typically 11+, varies by language)
  /// - other: fallback for any other cases
  /// 
  /// The developer controls which keys to use and their specific meaning per language.
  String _getPluralKey(String locale, int count) {
    // Universal pluralization logic - works for all languages
    if (count == 0) return 'zero';
    if (count == 1) return 'one';
    if (count == 2) return 'two';
    
    // Flexible support for 'few' and 'many'
    // Developers can customize these ranges in their JSON files
    if (count >= 3 && count <= 10) return 'few';
    if (count >= 11) return 'many';
    
    // Fallback to 'other' for any edge cases
    return 'other';
  }
}
''';
